    ...
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="dialog-content fixed left-1/2 top-1/2 w-[460px] -translate-x-1/2 -translate-y-1/2 rounded-xl border border-neutral-800 bg-neutral-900 p-5 shadow-xl shadow-black/40 focus:outline-none">
          <Dialog.Title className="text-base font-semibold text-neutral-100">Create Index</Dialog.Title>
          ...
          {preview === null ? (
            <div className="mt-4 space-y-3">
              <label className="block">
                <span className="mb-1 block text-xs font-medium text-neutral-500">Index name</span>
                <input
                  value={name}
              ...
              <label className="flex items-center gap-2 pt-1">
                ...
                />
                <span className="text-xs text-neutral-400">Unique index</span>
              </label>
              <div>
                <span className="mb-1 block text-xs font-medium text-neutral-500">Columns</span>
                <div className="max-h-40 space-y-1 overflow-y-auto rounded-lg border border-neutral-800 bg-neutral-950 p-2">
                  {columns.map((c) => (
                    <label key={c} className="flex items-center gap-2">
                      <input
                        ...
                        className="accent-indigo-500"
                      />
                      <span className="font-mono text-xs text-neutral-300">{c}</span>
                    </label>
                  ))}
                </div>
              </div>
