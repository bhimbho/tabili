  ...
  return (
    <Panel>
      <div className="mb-2 flex items-center justify-between">
        <h3 className="text-xs font-semibold uppercase tracking-wide text-(--text-faint)">Triggers</h3>
        <button
      ...
      {triggers && triggers.length > 0 ? (
        <DataTable head={["Name", "Timing", "Event", "Definition"]}>
          {triggers.map((t) => (
            <tr key={t.name} className="text-(--text-muted) align-top">
              <td className="whitespace-nowrap px-3 py-1.5 font-medium">{t.name}</td>
              <td className="whitespace-nowrap px-3 py-1.5 text-(--text-faint)">{t.timing || "—"}</td>
              <td className="whitespace-nowrap px-3 py-1.5 text-(--text-faint)">{t.event || "—"}</td>
              <td className="selectable max-w-[420px] px-3 py-1.5 font-mono text-[11px] text-(--text-muted)">
